...
def load_data_from_notion():
    ...
    for row in results:
        ...
        try:
            detail = props["상세내역"]["title"][0]["text"]["content"] if props["상세내역"]["title"] else ""
            date = props["날짜"]["date"]["start"] if props["날짜"]["date"] else None
            recorder = props["기록자"]["select"]["name"] if props["기록자"]["select"] else ""
            type_choice = props["구분"]["select"]["name"] if props["구분"]["select"] else ""
            category = props["대분류"]["select"]["name"] if props["대분류"]["select"] else ""
            amount = props["금액"]["number"] if props["금액"]["number"] else 0
            payment = props["결제수단"]["select"]["name"] if props["결제수단"]["select"] else ""
...
def save_to_notion(date, recorder, type_choice, category, detail, amount, payment):
    url = "https://api.api.notion.com/v1/pages" # 안전한 API 주소
    ...
    if user_text:
        ...
        if parsed_data:
            ...
            if success:
                ...
                st.sidebar.error("노션 저장 실패! 키와 ID를 확인하세요.")
        else:
